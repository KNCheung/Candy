...
; 测试平台：Win10
; 原作者：		妖(aamii@qq.com)
; 修改者： Beobachter70 (Beobachter70@163.com)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 /*
╔══════════════════════════════════════╗
<<<<〇程序头部>>>>                                                  ║
...
global CandyStartTick
if %2%
{
    tmp=%2%
    CandyStartTick := RegExReplace(tmp,"i).Tick=")
}
else
    CandyStartTick := A_TickCount
...
Label_Candy_ReadConfig_File:
    if %0%
        tmp = %1%
    else
        tmp = Candy.ini
    Candy_ProFile_Ini := RegExReplace(tmp,".ini=")
    Candy_ProFile_Ini = %A_ScriptDir%\%Candy_ProFile_Ini%
    global GeneralSettings_ini
    GeneralSettings_ini = %Candy_ProFile_Ini%
...
Label_Candy_ReadMyVar:
    global szMenuIdx:={}      ;菜单用1
    global szMenuContent:={}      ;菜单用2
    global szMenuWhichFile:={}      ;菜单用3
    IniRead All_MyVar,%Candy_ProFile_Ini%,MyVar		   	;读取我的变量，进行环境变量设置
...
Label_Candy_DrawMenu:
 ...
 ;加第一行菜单，缩略显示选中的内容，该菜单让你拷贝其内容
    ...
    MouseMove,CandyMenu_X,CandyMenu_Y,0
     ;traytip,candy,% A_TickCount-CandyStartTick,200,0     ;若要评估出menu时间，这里需打开 ,共三处，2/3
    Candy_GenerateTime := A_TickCount-CandyStartTick
	If (SkSub_IniRead(Candy_ProFile_Ini,"Candy_Settings","ShowTime", 0)=1 )     ; 打开时间评估？
    {
        CandyItem = Candy-%CandySel_Ext%`t%Candy_GenerateTime%ms
        Menu,CandyTopLevelMenu,Add
        Menu,CandyTopLevelMenu,Add,%CandyItem%,DoNothing
